...
export async function getHistoryItems(params: { reviewStatus: ReviewStatus }) {
  const res = await axiosInstance.get<HistoryItemsResponse>('/histories/items', { params });
...
export async function getWrittenReviews() {
  const res = await axiosInstance.get<WrittenReviewsResponse>('/histories/reviews');
