...
export async function createChat(payload: CreateChatRequest): Promise<ChatRoom> {
  const res = await instance.post<ChatRoom | ApiWrappedSuccess<ChatRoom> | ApiWrappedFail>('/chats', payload, {
...
export async function getChatRooms(): Promise<ChatRoomListItem[]> {
  const res = await instance.get<unknown>('/chats', { withCredentials: true });
  const body = res.data;
...
export async function getChatRoomDetail(chatId: number): Promise<ChatRoomDetail> {
  const res = await instance.get<ChatRoomDetail | ApiWrappedSuccess<ChatRoomDetail> | ApiWrappedFail>(
    `/chats/${chatId}`,
    {
      withCredentials: true,
    },
...
export async function getChatQuestion(chatId: number): Promise<ChatQuestionResponse> {
  const res = await instance.get<ChatQuestionResponse | ApiWrappedSuccess<ChatQuestionResponse> | ApiWrappedFail>(
...
export async function postChatSelect(
  ...
  const res = await instance.post<ApiWrappedSuccess<unknown> | ApiWrappedFail | unknown>(
    `/chats/${chatId}/select`,
    payload,
    {
      withCredentials: true,
...
export async function getChatResult(chatId: number): Promise<ChatResult> {
  const res = await instance.get<ChatResult | ApiWrappedSuccess<ChatResult> | ApiWrappedFail>(
    `/chats/${chatId}/result`,
    {
      withCredentials: true,
    },
...
export async function deleteChatRoom(chatId: number): Promise<void> {
  const res = await instance.delete<unknown>(`/chats/${chatId}`, { withCredentials: true });
  const body = res.data;
