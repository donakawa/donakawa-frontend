...
export async function getAuthMe(): Promise<ApiResponse<MeData>> {
  const { data } = await instance.get<ApiResponse<MeData>>('/auth/me');
  return data;
}
...
export async function sendEmailCode(body: { email: string; type?: EmailCodeType }): Promise<ApiResponse<null>> {
  ...
  const { data } = await instance.post<ApiResponse<null>>('/auth/email/send-code', payload);
  return data;
}
...
export async function verifyEmailCode(body: {
  ...
  const { data } = await instance.post<ApiResponse<{ token?: string }>>('/auth/email/verify-code', payload);
  return data;
}
...
export async function patchPassword(body: { newPassword: string }): Promise<ApiResponse<null>> {
  const { data } = await instance.patch<ApiResponse<null>>('/auth/password', body);
  return data;
}

// 회원 탈퇴
export async function deleteAccount(): Promise<ApiResponse<null>> {
  const { data } = await instance.delete<ApiResponse<null>>('/auth/account');
  return data;
}
